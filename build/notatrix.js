...
var Sentence = function (_Object) {
  ...
  _createClass(Sentence, [{
    ...
    key: 'getAnalysis',
    value: function getAnalysis(index) {
      var tok = this.getToken(index);
    ...
    set: function set(conllu) {
      ...
      for (var i = 0; i < lines.length; i++) {
        if (regex.comment.test(lines[i])) {
          ...
          for (var j = subTokenIndices[0]; j <= subTokenIndices[1]; j++) {
            var subToken = new Token(this);
    ...
    get: function get() {
      try {
        var params = [];
        this.forEach(function (token) {
          if (token.isSuperToken || token.isSubToken) throw new E.InvalidCoNLLUError();
          if (token.isAmbiguous) throw new E.InvalidCG3Error();
...
var Token = function (_Object) {
  ...
  _createClass(Token, [{
    ...
    value: function insertSubToken(index, token) {
      ...
      var subTokens = this.analysis.subTokens;
      this.analysis.subTokens = subTokens.slice(0, index).concat(token).concat(subTokens.slice(index));
    }
  }, {
    key: 'removeSubToken',
    value: function removeSubToken(index) {}
    ...
    value: function index(id) {
      if (this.isSuperToken) {
        ...
        _.each(this.analysis.subTokens, function (subToken) {
