...
function default_1(sent, options) {
    ...
    var output = sent.tokens.map(function (token) {
        if (token.analysis)
            throw new utils_1.GeneratorError("Unable to generate, contains ambiguous analyses or multiword tokens", sent, options);
        var params = {};
        utils_1.C.fields.forEach(function (field) {
            params[field] = token[field];
        });
        params.head = token.getHead(null);
        return _.pick(params, function (value) { return value != undefined; });
    });
    return {
        output: output,
