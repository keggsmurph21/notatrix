...
module.exports = (text, options) => {
  ...
  if (!text && !options.allowEmptyString)
    throw new ParserError(`Illegal CoNLL-U: empty string`, text, options);

  if (funcs.isJSONSerializable(text))
    throw new ParserError(`Illegal CoNLL-U: JSON object`, text, options);
  ...
  lines.forEach((line, i) => {
    ...
    if (re.comment.test(line)) {
      ...
      if (doneComments)
        throw new ParserError(`Illegal CoNLL-U: misplaced comment`, text, options);
      ...
      if (line) {
        if (!tokenLine.test(line))
          throw new ParserError(`Illegal CoNLL-U: unmatched line`, text, options);

        if (doneContent)
          throw new ParserError(`Illegal CoNLL-U: misplaced whitespace`, text, options);

      } else {

        // only allow empty lines after we've looked at all the content
        if (!options.allowTrailingWhitespace)
          throw new ParserError(`Illegal CoNLL-U: contains trailing whitespace`, text, options);
