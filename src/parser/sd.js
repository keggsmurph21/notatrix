...
module.exports = (text, options) => {
  ...
  if (!text && !options.allowEmptyString)
    throw new ParserError(`Illegal SD: empty string`, text, options);

  if (funcs.isJSONSerializable(text))
    throw new ParserError(`Illegal SD: JSON object`, text, options);
  ...
  lines.forEach((line, i) => {
    ...
    if (re.whiteline.test(line)) {
      if (parsingDeps) {
        if (!options.allowTrailingWhitespace)
          throw new ParserError(`Illegal SD: contains trailing whitespace`, text, options);

      } else {
        if (!options.allowLeadingWhitespace)
          throw new ParserError(`Illegal SD: contains leading whitespace`, text, options);
    ...
    if (re.comment.test(line)) {
      ...
      if (dependencyRegex.test(line))
        throw new ParserError(`Illegal SD: missing text line`, text, options);

      parsingDeps = true;

    } else if (!dependencyRegex.test(line)) {

      throw new ParserError(`Illegal SD: expected dependency line`, text, options);
  ...
  if (parsedDeps === 0 && !options.allowNoDependencies)
    throw new ParserError(`Illegal SD: contains no dependencies`, text, options);
