...
describe('relation sets', () => {
  ...
  it(`should only keep one head at a time if not enhanced`, () => {
    ...
    expect(ret2).to.equal(true); // because not enhanced, clear every time we add
  ...
  it(`should remove a dependency if it exists`, () => {
    ...
    expect(ret.token instanceof nx.BaseToken && typeof ret.deprel === 'string').to.equal(true);
  ...
  it(`should output correct head, deprel, deps fields for un-enhanced`, () => {
    ...
    const sent = new nx.Sentence('first second third fourth', { enhanced: false });
